...
//        lv_canvas_set_buffer(qr_canvas, LvglDriver::instance().get_canvas_buf(), QR_CODE_MAX_LEN, QR_CODE_MAX_LEN,
//                             LV_IMG_CF_TRUE_COLOR);

        desc_label = lv_label_create(m_scr);
        lv_obj_align_to(desc_label, qr_rect, LV_ALIGN_OUT_BOTTOM_MID, 0, 8);
        Lang::Language::instance().lang_set_style_text_font(desc_label,
    ...
    void UIQrCode::update_qr(const char *qr_content)
    {
        ...
        //lv_canvas_set_buffer(qr_canvas, LvglDriver::instance().get_canvas_buf(), side, side, LV_IMG_CF_TRUE_COLOR);
        lv_obj_align(qr_canvas, LV_ALIGN_CENTER, 0, 0);
...
//    void UIQrCode::routine() {
//        Base::routine();
//        /* fixme set max fade */
//        Bck::instance().fade_to_end(true, false, 200);
//    }
}