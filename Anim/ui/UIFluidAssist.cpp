...
namespace UI {
    UIFluidAssist::UIFluidAssist(int tank_size)
        ...
        Lang::Language::instance().lang_set_style_text_font(m_middle_label,
    ...
    void UIFluidAssist::start_routine() {
        update_assist_bar();
        std::string temp = LANG_FORMAT<int>(Lang::ui_fluid_add_about, {50});
...
//        }
    ...
    void UIFluidAssist::stable_cb(bool stable) {
        ...
        if (stable) {
            ...
            if (m_current_y > m_assist_bottom) {
                lv_label_set_text(m_middle_label,
                                  Lang::Text::instance().get_text(Lang::ui_fluid_not_yet).c_str());
                lv_obj_set_style_text_color(m_middle_label, get_palette_rgb(palette_notice), 0);
            } else if (m_current_y < m_assist_top) {
                lv_label_set_text(m_middle_label,
                                  Lang::Text::instance().get_text(Lang::ui_fluid_too_much).c_str());
                ...
                lv_label_set_text(m_middle_label,
                                  Lang::Text::instance().get_text(Lang::ui_fluid_good).c_str());
